...
void RKR::midievents() {
	...
	if ((Tap_Bypass) && (Tap_Selection == 3)
			&& (midievent->type == SND_SEQ_EVENT_CLOCK)) {
		mtc_counter++;
		if (mtc_counter >= 24) {
	...
	if ((midievent->type == SND_SEQ_EVENT_NOTEON)
		...
		if ((Tap_Bypass) && (Tap_Selection == 1)
				&& (midievent->type == SND_SEQ_EVENT_NOTEON) && (cmdvelo != 0))
			Tap_TempoSet = TapTempo();
			...
			if (!midi_table) {
				...
				if (midievent->data.control.value == 81)
					if (Selected_Preset > 1)
						preset = Selected_Preset - 1;

				if (midievent->data.control.value == 82)
					if (Selected_Preset < 60)
						preset = Selected_Preset + 1;
			} else
				preset = midievent->data.control.value;
			...
			if (MIDIway) {
				for (i = 0; i < 20; i++) {
					if (XUserMIDI[(int) midievent->data.control.param][i])
						process_midi_controller_events(
								XUserMIDI[(int) midievent->data.control.param][i],
				...
				process_midi_controller_events(
						(int) midievent->data.control.param,
						(int) midievent->data.control.value);
...
int RKR::checkonoff(int miraque) {
...
void RKR::jack_process_midievents(jack_midi_event_t *midievent) {

	int i;
	int type = midievent->buffer[0] >> 4;

	if ((Tap_Bypass) && (Tap_Selection == 3)
			&& (midievent->buffer[0] == 0xf8)) {
		mtc_counter++;
		if (mtc_counter >= 24) {
			Tap_TempoSet = TapTempo();
			mtc_counter = 0;
	...
	if ((type == 8) || (type == 9)) {
		...
		if ((Tap_Bypass) && (Tap_Selection == 1) && (type == 9)
				&& (cmdvelo != 0))
			Tap_TempoSet = TapTempo();
			...
			if (MIDIway) {
				for (i = 0; i < 20; i++) {
					if (XUserMIDI[cmdcontrol][i])
						process_midi_controller_events(XUserMIDI[cmdcontrol][i],
								cmdvalue);
					else
						break;
				}
 ...
 */
void RKR::process_midi_controller_events(int parameter, int value) {
	...
	case 7:
		Master_Volume = (float) value / 128.0f;
		calculavol(2);
		break;
	...
	case 142:
		efx_Compressor->Compressor_Change(4,
				10 + (int) ((float) value * 1.890f));
		break;

	case 143:
		efx_Compressor->Compressor_Change(5,
				10 + (int) ((float) value * 3.858f));
		break;
	...
	case 146:
		efx_Compressor->Compressor_Change(1,
				-60 + (int) ((float) value * .448f));
		break;

	case 147:
		efx_Compressor->Compressor_Change(3,
				-40 + (int) ((float) value * .314f));
		break;
	...
	case 220:
		efx_Expander->Expander_Change(3,
				10 + (int) ((float) value * 15.669291f));
		break;

	case 221:
		efx_Expander->Expander_Change(4,
				10 + (int) ((float) value * 3.8582677f));
		break;
	...
	case 360:
		efx_StereoHarm->changepar(3,
				-2000 + (int) ((float) value * 31.496063f));
		break;
...
int RKR::ret_Tempo(int value) {
	return (1 + (int) ((float) value * 4.724f));
}
int RKR::ret_LPF(int value) {
	return (20 + (int) ((float) value * 204.566f));
}
int RKR::ret_HPF(int value) {
