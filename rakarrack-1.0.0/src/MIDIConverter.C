...
MIDIConverter::MIDIConverter() {
	...
	schmittBuffer = NULL;
	schmittPointer = NULL;
	static const char *englishNotes[12] = { "A", "A#", "B", "C", "C#", "D",
			"D#", "E", "F", "F#", "G", "G#" };
	notes = englishNotes;
	note = 0;
	nfreq = 0;
...
void MIDIConverter::displayFrequency(float ffreq) {
	int i;
	int noteoff = 0;
	int octave = 4;
...
void MIDIConverter::schmittInit(int size) {
	blockSize = SAMPLE_RATE / size;
	schmittBuffer = (signed short int *) malloc(
			blockSize * sizeof(signed short int));
	schmittPointer = schmittBuffer;
}
;

void MIDIConverter::schmittS16LE(int nframes, signed short int *indata) {
	int i, j;
	float trigfact = 0.6f;
		...
		*schmittPointer++ = indata[i];
		if (schmittPointer - schmittBuffer >= blockSize) {
			...
			for (j = 1; schmittBuffer[j] <= t1 && j < blockSize; j++)
				;
			for (;
					!(schmittBuffer[j] >= t2 && schmittBuffer[j + 1] < t2)
							&& j < blockSize; j++)
				;
			startpoint = j;
			schmittTriggered = 0;
			...
			if (endpoint > startpoint) {
				afreq = fSAMPLE_RATE
						* ((float) tc / (float) (endpoint - startpoint));
...
void MIDIConverter::schmittFree() {
	free(schmittBuffer);
}
;
...
void MIDIConverter::setmidichannel(int chan) {
	channel = chan;

}
;

void MIDIConverter::setTriggerAdjust(int val) {
...
void MIDIConverter::setVelAdjust(int val) {
