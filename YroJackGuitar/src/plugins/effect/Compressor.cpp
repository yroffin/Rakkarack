...
Compressor::Compressor() :
		YroEffectPlugin("Compressor",
				...
				"HarmonicEnhancer: -20,15,-3,5,50,0,0,1,1;"
				"BandCompBand: -3,2,0,5,50,1,0,1,0;"
				"EndCompBand: -60,2,0,10,500,1,0,1,1;") {
	rvolume = 0.0f;
	rvolume_db = 0.0f;
	lvolume = 0.0f;
	lvolume_db = 0.0f;
...
//Right Channel
		...
		if (stereo) {
			...
			if (rvolume_db < thres_db) {
				...
				eratio = 1.0f
						+ (kratio - 1.0f) * (rvolume_db - thres_db)
								* coeff_knee;
				rgain =
						outlevel
								* dB2rap(thres_db + (rvolume_db-thres_db)/eratio - rvolume_db);
			} else {
				rgain =
						outlevel
								* dB2rap(thres_db + coeff_kk + (rvolume_db-thres_mx)*coeff_ratio - rvolume_db);
				limit = 1;
			}
...
//Left Channel
		...
		if (lvolume_db < thres_db) {
			lgain = outlevel;
				...
				{
			eratio = 1.0f
					+ (kratio - 1.0f) * (lvolume_db - thres_db) * coeff_knee;
			lgain =
					outlevel
							* dB2rap(thres_db + (lvolume_db-thres_db)/eratio - lvolume_db);
		} else {
			lgain =
					outlevel
							* dB2rap(thres_db + coeff_kk + (lvolume_db-thres_mx)*coeff_ratio - lvolume_db);
			limit = 1;
		}
