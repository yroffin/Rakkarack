 ...
 * Effect output
 */
void Reverbtron::render(jack_nframes_t nframes, float * smpsl, float * smpsr) {
...
int Reverbtron::setfile(int value) {
	int i;
...
//Time Data
	...
	for (i = 0; i < data_length; i++) {
		if (ftime[i] > maxtime)
			maxtime = ftime[i];
		if (tdata[i] > maxdata)
			maxdata = tdata[i]; //used to normalize so feedback is more predictable
		if (i > 0) {
		...
		for (i = 0; i < data_length; i++) {
			...
			if (findex < skip) {
				if (index < Plength) {
					...
					time[index] = lrintf(
							tmpstretch * (idelay + ftime[i]) * nfSAMPLE_RATE); //Add initial delay to all the samples
...
//generate an approximated randomized hrtf for diffusing reflections:
	int tmptime = 0;
	int hrtf_tmp = Pdiff;
	if (hrtf_tmp > data_length)
		hrtf_tmp = data_length - 1;
	if (hlength > data_length)
		hlength = data_length - 1;
 ...
 * toXml member
*/
const char *Reverbtron::toXml() {
        char _buffer[256];
        char _formatd[] = {"<attribute name=\"%s\" value=\"%d\" />"};
        char _formatf[] = {"<attribute name=\"%s\" value=\"%9.40f\" />"};
        strcpy(_toXml,"<attributes>");
        sprintf(_buffer,_formatd,"avgtime",avgtime);
        strcat(_toXml,_buffer);
        sprintf(_buffer,_formatd,"data_length",data_length);
        strcat(_toXml,_buffer);
        sprintf(_buffer,_formatd,"DS_state",DS_state);
        strcat(_toXml,_buffer);
        sprintf(_buffer,_formatd,"Filenum",Filenum);
        strcat(_toXml,_buffer);
        sprintf(_buffer,_formatd,"hlength",hlength);
        strcat(_toXml,_buffer);
        sprintf(_buffer,_formatd,"hoffset",hoffset);
        strcat(_toXml,_buffer);
        sprintf(_buffer,_formatd,"hrtf_size",hrtf_size);
        strcat(_toXml,_buffer);
        sprintf(_buffer,_formatd,"imax",imax);
        strcat(_toXml,_buffer);
        sprintf(_buffer,_formatd,"imctr",imctr);
        strcat(_toXml,_buffer);
        ...
        strcat(_toXml,_buffer);
        sprintf(_buffer,_formatd,"Pdiff",Pdiff);
        strcat(_toXml,_buffer);
        sprintf(_buffer,_formatd,"Pes",Pes);
        strcat(_toXml,_buffer);
        sprintf(_buffer,_formatd,"Pfade",Pfade);
        strcat(_toXml,_buffer);
        ...
        strcat(_toXml,_buffer);
        sprintf(_buffer,_formatd,"Prv",Prv);
        strcat(_toXml,_buffer);
        sprintf(_buffer,_formatd,"Psafe",Psafe);
        strcat(_toXml,_buffer);
        sprintf(_buffer,_formatd,"Pstretch",Pstretch);
        strcat(_toXml,_buffer);
        sprintf(_buffer,_formatd,"Puser",Puser);
        strcat(_toXml,_buffer);
        sprintf(_buffer,_formatd,"Pvolume",Pvolume);
        strcat(_toXml,_buffer);
        sprintf(_buffer,_formatf,"fstretch",fstretch);
        strcat(_toXml,_buffer);
        sprintf(_buffer,_formatf,"idelay",idelay);
        strcat(_toXml,_buffer);
        sprintf(_buffer,_formatf,"ffade",ffade);
        strcat(_toXml,_buffer);
        sprintf(_buffer,_formatf,"maxtime",maxtime);
        strcat(_toXml,_buffer);
        sprintf(_buffer,_formatf,"maxdata",maxdata);
        strcat(_toXml,_buffer);
        sprintf(_buffer,_formatf,"decay",decay);
        strcat(_toXml,_buffer);
        sprintf(_buffer,_formatf,"diffusion",diffusion);
        strcat(_toXml,_buffer);
