...
StompBox::StompBox() :
		YroEffectPlugin("StompBox", "Odie: 80, 32, 0, 32, 10, 0;"
				...
				"Classic Dist: 50, 64, 0, 0, 110, 4;"
				"Morbid Impalement: 38, 6, 6, 6, 105, 5;"
				"Mid Elve: 48, 0, -12, 0, 127, 6;"
				"Fuzz: 48, 0, 0, 0, 127, 7;") {
 ...
 * Effect output
 */
void StompBox::render(jack_nframes_t nframes, float * smpsl, float * smpsr) {
	int i;
	...
	case 0: //Odie
		...
		lpre2->filterout(iPERIOD, fPERIOD, smpsl);
		rpre2->filterout(iPERIOD, fPERIOD, smpsr);
		rwshape->waveshapesmps(iPERIOD, smpsl, 28, 20, 1); //Valve2
		lwshape->waveshapesmps(iPERIOD, smpsr, 28, 20, 1);
		ranti->filterout(iPERIOD, fPERIOD, smpsr);
		lanti->filterout(iPERIOD, fPERIOD, smpsl);
		lpre1->filterout(iPERIOD, fPERIOD, smpsl);
		rpre1->filterout(iPERIOD, fPERIOD, smpsr);
		rwshape2->waveshapesmps(iPERIOD, smpsl, 28, Pgain, 1); //Valve2
		lwshape2->waveshapesmps(iPERIOD, smpsr, 28, Pgain, 1);

		lpost->filterout(iPERIOD, fPERIOD, smpsl);
		rpost->filterout(iPERIOD, fPERIOD, smpsr);
		...
		for (i = 0; i < iPERIOD; i++) {
			...
			efxoutl[i] = 0.5f * volume
					* (smpsl[i] + lowb * lfilter + midb * mfilter
							+ highb * hfilter);
			...
			efxoutr[i] = 0.5f * volume
					* (smpsr[i] + lowb * lfilter + midb * mfilter
							+ highb * hfilter);
	...
	case 5: //Death Metal
	case 6: //Metal Zone
		linput->filterout(iPERIOD, fPERIOD, smpsl);
		rinput->filterout(iPERIOD, fPERIOD, smpsr);
		...
		rwshape->waveshapesmps(iPERIOD, smpsl, 24, 1, 1); // Op amp limiting
		lwshape->waveshapesmps(iPERIOD, smpsr, 24, 1, 1);

		ranti->filterout(iPERIOD, fPERIOD, smpsr);
		lanti->filterout(iPERIOD, fPERIOD, smpsl);

		rwshape2->waveshapesmps(iPERIOD, smpsl, 23, Pgain, 1); // hard comp
		lwshape2->waveshapesmps(iPERIOD, smpsr, 23, Pgain, 1);
	...
	case 3: //Fat Cat  //Pre gain & filter freqs the only difference
		...
		rwshape2->waveshapesmps(iPERIOD, smpsl, 23, 1, 0); // hard comp
		lwshape2->waveshapesmps(iPERIOD, smpsr, 23, 1, 0);
	...
	case 4: //Dist+
		...
		rwshape2->waveshapesmps(iPERIOD, smpsl, 29, 1, 0); // diode limit
		lwshape2->waveshapesmps(iPERIOD, smpsr, 29, 1, 0);
	...
	case 7: //Classic Fuzz
		...
		linput->filterout(iPERIOD, fPERIOD, smpsl);
		rinput->filterout(iPERIOD, fPERIOD, smpsr);
		rwshape->waveshapesmps(iPERIOD, smpsr, 19, 25, 1); //compress
		lwshape->waveshapesmps(iPERIOD, smpsl, 19, 25, 1);
		...
		ranti->filterout(iPERIOD, fPERIOD, smpsr);
		lanti->filterout(iPERIOD, fPERIOD, smpsl);
		rwshape2->waveshapesmps(iPERIOD, smpsr, 25, Pgain, 1); //JFET
		lwshape2->waveshapesmps(iPERIOD, smpsl, 25, Pgain, 1);
		lpre2->filterout(iPERIOD, fPERIOD, smpsl);
		rpre2->filterout(iPERIOD, fPERIOD, smpsr);
...
void StompBox::setHigh(int value) {
	Phigh = value;
	if (value < 0)
...
void StompBox::setMid(int value) {
	Pmid = value;
	if (value < 0)
...
void StompBox::setLow(int value) {
	Plow = value;
	if (value < 0)
...
void StompBox::setGain(int value) {
	Pgain = value;
	gain = dB2rap(50.0f * ((float)value)/127.0f - 50.0f);
	init_tone();
}
void StompBox::setMode(int value) {
	Pmode = value;
	init_mode(Pmode);
	init_tone();
}

int StompBox::getVolume() {
	return Pvolume;
}
int StompBox::getHigh() {
	return Phigh;
}
int StompBox::getMid() {
	return Pmid;
...
int StompBox::getGain() {
	return Pgain;
}
int StompBox::getMode() {
	return Pmode;
}
