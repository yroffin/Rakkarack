/*
 ...
 YroLowfrequencyOscillation.C - Stereo LFO used by some effects
 ...
 */
YroLowfrequencyOscillation::YroLowfrequencyOscillation() :
		YroEffectPlugin("LowfrequencyOscillation") {
	xl = 0.0;
	xr = 0.0;
	Pfreq = 40;
	Prandomness = 0;
 ...
 */
void YroLowfrequencyOscillation::updateparams() {
 ...
 */
float YroLowfrequencyOscillation::getlfoshape(float x) {
	float tmpv;
	float out = 0.0;
	...
	case 7: // Lorenz Fractal
		...
		if (lfotype == 7) {
			if ((radius = (sqrtf(x0 * x0 + y0 * y0 + z0 * z0) * scale) - 0.25f)
					> 1.0f)
				radius = 1.0f;
			if (radius < 0.0)
				radius = 0.0;
 ...
 * LFO output
 */
void YroLowfrequencyOscillation::render(jack_nframes_t nframes, float * outl, float * outr) {
	float out;
...
int YroLowfrequencyOscillation::getPfreq() const {
	return Pfreq;
}

void YroLowfrequencyOscillation::setPfreq(int pfreq) {
	Pfreq = pfreq;
}

int YroLowfrequencyOscillation::getPlfOtype() const {
	return PLFOtype;
}

void YroLowfrequencyOscillation::setPlfOtype(int plfOtype) {
	PLFOtype = plfOtype;
	updateparams();
}

int YroLowfrequencyOscillation::getPrandomness() const {
	return Prandomness;
}

void YroLowfrequencyOscillation::setPrandomness(int prandomness) {
	Prandomness = prandomness;
	updateparams();
}

int YroLowfrequencyOscillation::getPstereo() const {
	return Pstereo;
}

void YroLowfrequencyOscillation::setPstereo(int pstereo) {
	Pstereo = pstereo;
	updateparams();
}
