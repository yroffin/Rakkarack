/*
 ...
 YroLowfrequencyOscillation.h - Stereo LFO used by some effects
...
class YroLowfrequencyOscillation: public YroEffectPlugin {
public:
	YroLowfrequencyOscillation();
	~YroLowfrequencyOscillation();
	void render(jack_nframes_t nframes, float *outl, float *outr);
	void updateparams();
	int getPfreq() const;
	void setPfreq(int pfreq);
	int getPlfOtype() const;
	void setPlfOtype(int plfOtype);
	int getPrandomness() const;
	void setPrandomness(int prandomness);
