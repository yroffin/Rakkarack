...
HarmEnhancer::HarmEnhancer(float *Rmag, float hfreq, float lfreq, float gain) {
	/**
	 * TODO spy this suspicious allocation
	 */
...
void HarmEnhancer::set_vol(int mode, float gain) {
	if (!mode)
		vol = gain;
	else
		vol = realvol + gain;
...
void HarmEnhancer::set_freqh(int mode, float freq) {
	if (!mode) {
		hpffreq = freq;
		freq = 0.0;
	}
...
void HarmEnhancer::set_freql(int mode, float freq) {
	if (!mode) {
		lpffreq = freq;
		freq = 0.0;
	}
 ...
 * example in Num. Rec. */
void HarmEnhancer::chebpc(float c[], float d[]) {
	int j, k;
...
void HarmEnhancer::calcula_mag(float *Rmag) {

	int i;
	float mag_fix = 0.0f;

	float mag[HARMONICS] = { 0.0f, Rmag[0], Rmag[1], Rmag[2], Rmag[3], Rmag[4],
			Rmag[5], Rmag[6], Rmag[7], Rmag[8], Rmag[9] };
...
void HarmEnhancer::harm_out(jack_nframes_t nframes, float *smpsl, float *smpsr) {
	/**
	 * TODO harmEnhancer seem to be an effect ...
	 * TODO spy this memcpy
	 */
	int j;

	memcpy(inputl, smpsl, sizeof(float) * iPERIOD);
	memcpy(inputr, smpsr, sizeof(float) * iPERIOD);
	...
	lpfl->filterout(iPERIOD, fPERIOD, inputl);
	lpfr->filterout(iPERIOD, fPERIOD, inputr);
