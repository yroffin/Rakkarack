...
#include <math.h>
#include "YroWaveShaper.h"

YroWaveShaper::YroWaveShaper() {
	YroParamHelper *params = YroParamHelper::instance();

	switch (params->getWaveshapeResampling()) {
	case 0:
		period_coeff = 1;
		ncSAMPLE_RATE = params->getInverseFloatSampleRate();
		break;
	case 1:
		period_coeff = 2;
		ncSAMPLE_RATE = params->getInverseFloatSampleRate() / 2.0f;
		break;
	case 2:
		period_coeff = 4;
		ncSAMPLE_RATE = params->getInverseFloatSampleRate() / 4.0f;
		break;
	case 3:
		period_coeff = 8;
		ncSAMPLE_RATE = params->getInverseFloatSampleRate() / 8.0f;
		break;
	case 4:
		period_coeff = 12;
		ncSAMPLE_RATE = params->getInverseFloatSampleRate() / 12.0f;
		break;
	}
	...
	U_Resample = new YroResample(params->getWaveshapeUpQuality()); //Downsample, uses sinc interpolation for bandlimiting to avoid aliasing
	D_Resample = new YroResample(params->getWaveshapeDownQuality());
}

YroWaveShaper::~YroWaveShaper() {
}

void YroWaveShaper::cleanup() {
	compg = 0.0f; //used by compression distortion
	cratio = 0.25f; //used by compression for hardness
	tmpgain = 1.0f; // compression distortion temp variable
	dthresh = 0.25;
...
void YroWaveShaper::waveshapesmps(jack_nframes_t n, float * smps, int type,
		int drive, int eff) {
	YroParamHelper *params = YroParamHelper::instance();

	int nn = n;

	if (params->getWaveshapeResampling() > 0) {
		nn = n * period_coeff;
	...
	case 23: //Super Soft
		...
		for (i = 0; i < nn; i++) {
			...
			/**
			 * TODO suppress this nop operation
			 */
			if (temps[i] < factor) /* nop */
				;
