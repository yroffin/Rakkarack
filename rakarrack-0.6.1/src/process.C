...
class simulation {
	public:
		static simulation *instance() {
			if (__instance == 0) {
				__instance = new simulation();
			}
			return __instance;
		}
		void toXmlRaw(FILE *out, char *name, int preset, float *inpl, float *inpr, float *outl, float *outr, unsigned int size) {
			MD5 *myMD5 = new MD5();
		    char hashil[1024];
		    char hashir[1024];
		    char hashol[1024];
		    char hashor[1024];
		    strcpy(hashil, myMD5->digestMemory((BYTE *)inpl, size * sizeof(float)));
		    strcpy(hashir, myMD5->digestMemory((BYTE *)inpr, size * sizeof(float)));
		    strcpy(hashol, myMD5->digestMemory((BYTE *)outl, size * sizeof(float)));
		    strcpy(hashor, myMD5->digestMemory((BYTE *)outr, size * sizeof(float)));
			fprintf(out,"<?xml version=\"1.0\"?><elements effect=\"%s\" preset=\"%d\" hashil=\"%s\" hashir=\"%s\" hashol=\"%s\" hashor=\"%s\">\n",
					name,
					preset,
					hashil,
					hashir,
			...
			for(unsigned i=0;i<size;i++) {
				fprintf(out,"<element index=\"%d\" ileft=\"%40.40f\" iright=\"%40.40f\" oleft=\"%40.40f\" oright=\"%40.40f\" />\n",
						i,
						inpl[i],
						inpr[i],
						outl[i],
		...
		void toXml(char *basename, int preset, float *inpl, float *inpr, float *outl, float *outr, unsigned int size) {
			char name[1024];
			sprintf(name,"%s-%08x.xml",basename,sequence++);
			FILE *out = fopen(name,"a");
			toXmlRaw(out, basename, preset, inpl,inpr,outl,outr,size);
			fclose(out);
		}
		void capture(Distorsion *my, int preset) {
			float *__efxoutl_copy;
			float *__efxoutr_copy;
			my->setpreset(1,preset);
			__efxoutl_copy = my->efxoutl;
			__efxoutr_copy = my->efxoutr;
			my->efxoutl = __efxoutl;
			my->efxoutr = __efxoutr;
			my->out(__efxinpl,__efxinpr);
			fprintf(stderr,"out=%f\n",my->efxoutl[444]);
			my->efxoutl = __efxoutl_copy;
			my->efxoutr = __efxoutr_copy;
			toXml("Distorsion", preset, __efxinpl, __efxinpr, __efxoutl, __efxoutr, PERIOD);
		}
	private:
		simulation() {
			__efxinpl = new float[PERIOD];
			__efxinpr = new float[PERIOD];
			__efxoutl = new float[PERIOD];
			__efxoutr = new float[PERIOD];

			for (int i = 0; i < 1024; i++) {
				sine[i] = .1f * (float) sin(((double) i / (double) 1024) * M_PI * 1.2f);
			}

			for (jack_nframes_t i = 0; i < (unsigned int) PERIOD; i++) {
				__efxinpl[i] = sine[i % 1024]; /* left */
				__efxinpr[i] = sine[(i + 512) % 1024]; /* right */
			}
		}

		static simulation *__instance;
		float *__efxinpl;
		float *__efxinpr;
		float *__efxoutl;
		float *__efxoutr;
...
simulation *simulation::__instance = 0;
      ...
      if((Ring_Bypass) && (efx_Ring->Pafreq))
        {
         ...
         simulation::instance()->capture(efx_Distorsion,0);
